...
```
demo (AppsPage)                                client host (FastAPI)
  callTool('open_counter_app') ──POST /api/tools/call──▶  api.call_tool ──▶ MCP server
      ▲                          ◀── result.content[resource] + _meta.ui.resourceUri      │
      │  render ui:// html in sandboxed <iframe sandbox="allow-scripts">         ui_tool_result
      │                                                                          ('ui://counter')
   postMessage bridge ◀────────────────────────────────────────────────────────────────┘
...
`resources/read` path used for any [resource](./resources.md).)
...
`ui_tool_result` helper — which stamps `_meta.ui.resourceUri` and includes the `ui://`
...
The HTML is self-contained — inlined in `counter_app.py` as the `COUNTER_APP_HTML` string —
and speaks the same `postMessage` bridge: it announces `ready`, emits `state`/`submit`, and
accepts `set`/`note` from the host:

```python
# py-mcp-server/counter_app.py
...
(The bridge body above is JavaScript inside the `COUNTER_APP_HTML` Python string — the app
is a self-contained HTML document, not part of the SPA's source.)
...
the namespaced `postMessage` bridge. See [Resources](./resources.md) for the embedded-resource
