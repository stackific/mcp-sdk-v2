...
with the C# stack on `Stackific.Mcp`, tracing the round-trip _from the demo to the MCP server
and back_.

The frontend is the shared SPA (TypeScript); selecting **C#** on the home page repoints it at
the C# client host. Only the client and server layers differ from the
...
| **MCP client** | `csharp-mcp-client/`       | Minimal API host of the `Stackific.Mcp` `McpClient`; REST → JSON-RPC over Streamable HTTP. |
| **MCP server** | `csharp-mcp-server/`       | Built on `Stackific.Mcp`; registers tools/resources/prompts/… |

```
demo route ──▶ demo/src/lib/api.ts ──REST──▶ csharp-mcp-client (Program.cs → ClientHost.cs)
                                                     │ Stackific.Mcp McpClient
