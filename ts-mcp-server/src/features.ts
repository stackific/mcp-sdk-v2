 ...
 */
import { McpServer, InMemoryTaskStore, withCacheHints, uiToolResult, UI_MIME_TYPE } from '@stackific/mcp-sdk-ts/server';
...
export function buildCompanionServer(): McpServer {
  ...
  server.registerTool(
    'echo',
    {
      ...
      inputSchema: { type: 'object', properties: { text: { type: 'string', description: 'Text to echo back' } }, required: ['text'] },
  ...
  server.registerTool(
    ...
    async (args) => {
      const conditions = (['sunny', 'cloudy', 'rainy', 'stormy'] as const)[Math.floor(Math.random() * 4)]!;
      const structuredContent = { city: args.city as string, tempC: Math.round((Math.random() * 30 - 5) * 10) / 10, conditions };
  ...
  server.registerTool(
    'register_user',
    { title: 'Register User (form elicitation)', description: 'Server requests user input via FORM elicitation.' },
    async (_args, ctx) => {
      ...
      return { content: [{ type: 'text', text: `User chose to ${String(result.action)} the form.` }] };
    },
  );

  server.registerTool(
    'confirm_purchase',
    { title: 'Confirm Purchase (URL elicitation)', description: 'Server requests confirmation via URL elicitation.' },
    async (_args, ctx) => {
      ...
      return { content: [{ type: 'text', text: `URL elicitation result: ${String(result.action)} (id=${elicitationId}).` }] };
    },
  );
  ...
  server.registerTool(
    ...
    async (args, ctx) => {
      ...
      return { content: [{ type: 'text', text: `Model "${String((message as { model?: string }).model)}" replied:\n${out}` }] };
    },
  );

  // Roots (server asks the client for its workspace).
  server.registerTool('show_roots', { title: 'Show Roots', description: 'Server requests the client roots list (roots/list).' }, async (_args, ctx) => {
    const result = (await ctx.listRoots()) as { roots?: unknown };
    return { content: [{ type: 'text', text: `Client roots:\n${JSON.stringify(result.roots, null, 2)}` }] };
  });
      ...
      for (; i < to; i++) {
        ...
        if (ctx.progressToken !== undefined) {
          ctx.notify({
            ...
            params: { progressToken: ctx.progressToken, progress: i + 1, total: to, message: `count ${i + 1}/${to}` },
          });
        }
        await delay(intervalMs);
      }
      const cancelled = ctx.signal.aborted;
      return { content: [{ type: 'text', text: cancelled ? `Cancelled at ${i}/${to}.` : `Counted to ${to}.` }] };
    },
  );
  ...
  server.registerTool(
    ...
    async (_args, ctx) => {
      ...
      ctx.notifySubscribers({ method: 'notifications/resources/updated', params: { uri: 'docs://readme' } });
  ...
  const CATALOG = Array.from({ length: 23 }, (_, i) => ({ id: i + 1, name: `item-${String(i + 1).padStart(2, '0')}` }));
  const PAGE_SIZE = 5;
  server.registerTool(
    'list_catalog',
    {
      ...
      inputSchema: { type: 'object', properties: { cursor: { type: 'string', description: 'Opaque cursor from a previous page' } } },
      outputSchema: {
        type: 'object',
        properties: {
    ...
    async (args) => {
      ...
      const nextCursor = nextOffset < CATALOG.length ? Buffer.from(String(nextOffset)).toString('base64') : undefined;
  ...
  let quoteCounter = 0;
  server.registerTool('cached_quote', { title: 'Cached Quote', description: 'Returns a result carrying top-level cache hints (ttlMs + cacheScope).' }, async () => {
    quoteCounter += 1;
    const quotes = ['Make it work, then make it right.', 'Cache invalidation is hard.', 'Premature optimization is the root of all evil.'];
    return withCacheHints(
      {
        content: [{ type: 'text', text: `#${quoteCounter}: ${quotes[quoteCounter % quotes.length]}` }],
  ...
  server.registerTool('echo_trace', { title: 'Echo Trace Context', description: 'Echoes back the _meta the server received (incl. traceparent/tracestate).' }, async (_args, ctx) => ({
  ...
  server.registerTool('content_gallery', { title: 'Content Gallery', description: 'Returns text, image, audio, an embedded resource, and a resource_link.' }, async () => ({
    content: [
  ...
  server.registerResource('counter-app', 'ui://counter', { title: 'Counter App (MCP Apps UI)', description: 'An interactive UI resource, rendered sandboxed by the host.', mimeType: UI_MIME_TYPE }, async (uri) => ({
    contents: [{ uri, mimeType: UI_MIME_TYPE, text: COUNTER_APP_HTML }],
  }));
  server.registerTool(
    'open_counter_app',
    { title: 'Open Counter App (MCP Apps)', description: 'Launches an MCP App: returns an embedded ui:// resource the host renders sandboxed.' },
    async () =>
  ...
  server.registerTool(
    ...
    {
      title: 'Long Job (task)',
      description: 'Runs as a task: returns a handle immediately, works through N steps, then exposes the result via tasks/result.',
      inputSchema: {
        type: 'object',
        properties: {
  ...
  server.registerResource('readme', 'docs://readme', { title: 'Readme', description: 'A static text resource.', mimeType: 'text/markdown' }, async (uri) => ({
    contents: [{ uri, mimeType: 'text/markdown', text: '# Companion Server\n\nThis is a static MCP resource served over Streamable HTTP.' }],
  }));
  ...
  server.registerResourceTemplate(
    ...
    async (uri, variables) => ({
      contents: [{ uri, mimeType: 'application/json', text: JSON.stringify({ city: variables.city, tempC: 21, conditions: 'sunny' }, null, 2) }],
    }),
  );
  ...
  server.registerPrompt(
    'greeting',
    {
      ...
      arguments: [
        ...
        { name: 'language', description: 'Language', complete: (value) => ['english', 'spanish', 'norwegian', 'japanese'].filter((l) => l.startsWith(value.toLowerCase())) },
      ],
    },
    async (args) => ({ messages: [{ role: 'user', content: { type: 'text', text: `Greet ${args.name} warmly in ${args.language ?? 'english'}.` } }] }),
  );
